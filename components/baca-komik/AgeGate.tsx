    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
      <div className="w-full max-w-sm rounded-2xl bg-zinc-900 p-6 ring-1 ring-white/10">
        ...
        <p className="mt-2 text-sm text-zinc-400">
          Situs ini memuat konten komik yang mungkin tidak sesuai untuk semua umur.
        ...
        <label className="mt-4 flex items-center gap-2 text-sm text-zinc-300">
          <input
            type="checkbox"
            checked={nsfw}
            onChange={(e) => setNsfwState(e.target.checked)}
            className="rounded"
          />
          Aktifkan mode NSFW (konten dewasa)
        </label>
        <div className="mt-5 flex gap-2">
          <button
            type="button"
            className="flex-1 rounded-xl bg-zinc-800 py-2.5 text-sm font-semibold text-zinc-300"
            onClick={() => {
              setAgeOk(false);
              setNsfw(false);
